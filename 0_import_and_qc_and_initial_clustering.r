...
# map these cells onto SI paper clusters
reference <- readRDS("/special_projects/55_scalp_psoriasis_analysis_jeff/RData/Manuscript_Object.rds")

## Find transfer anchors, transfer data and add metadata
SkinX.anchors <- FindTransferAnchors(reference = reference, query = merged_samples, dims = 1:30, reference.reduction = "pca")
predictionsX <- TransferData(anchorset = SkinX.anchors, refdata = reference$ID, dims=1:30)
merged_samples <- AddMetaData(merged_samples, metadata = predictionsX)

reference <- RunUMAP(reference, dims = 1:30, reduction = "pca", return.model = TRUE)
merged_samples <- MapQuery(anchorset = SkinX.anchors, reference = reference, query = merged_samples,
    refdata = list(ID = "ID"), reference.reduction = "pca", reduction.model = "umap")

pdf(paste0(output_dir,"/all_samples_mapped_to_SI_data_initial.pdf"), width=15, height=5)
p1 <- DimPlot(reference, reduction = "umap", label = TRUE, label.size = 3,
    repel = TRUE, raster = FALSE) + ggtitle("Reference annotations")
p2 <- DimPlot(merged_samples, reduction = "ref.umap", group.by = "predicted.ID", label = TRUE,
    label.size = 3, repel = TRUE, raster = FALSE) + ggtitle("Query transferred labels")
print(p1 + p2)
dev.off()

saveRDS(merged_samples, "/special_projects/55_scalp_psoriasis_analysis_jeff/RData/merged_all_samples_integrated_mapping_to_ref.RDS")

# (!) re-run all of the following lines if I want to re-generate the UMAPs (!)
# generate 41 colors to use them in plotting the reference and the query samples
n <- 41
qual_col_pals = brewer.pal.info[brewer.pal.info$category == 'qual',]
col_vector = unlist(mapply(brewer.pal, qual_col_pals$maxcolors, rownames(qual_col_pals)))
pie(rep(1,n), col=sample(col_vector, n))

reference$ID <- factor(reference$ID, levels=1:41)
col_vector <- col_vector[1:41]
names(col_vector) <- unique(reference$ID)

merged_samples$predicted.ID <- factor(merged_samples$predicted.ID, levels = levels(reference$ID))

pdf(paste0(output_dir,"/all_samples_mapped_to_SI_data_1.pdf"), width=15, height=8)
p1 <- DimPlot(reference, group.by="ID", reduction = "umap", label = TRUE, label.size = 3.5, pt.size=0.2,
	    repel = TRUE, raster = FALSE, cols=col_vector) + ggtitle("Reference") +NoLegend()+ylim(-15, 15)+xlim(-15,10)

p2 <- DimPlot(merged_samples, reduction = "ref.umap", group.by = "predicted.ID", label = TRUE, pt.size=0.2,
	    label.size = 3.5, repel = TRUE, raster = FALSE, cols=col_vector) + ggtitle("All samples") + NoLegend()+ylim(-15, 15)+xlim(-15,10)
print(p1 + p2)
dev.off()

# extract Trm1 (cluster 2) cells for downstream analysis
trm1 <- subset(merged_samples, subset = predicted.ID == "2")

trm1$sample %>% table()

p2 <- DimPlot(merged_samples, reduction = "ref.umap", group.by = "predicted.ID", label = TRUE, pt.size=0.2,
	    label.size = 3.5, repel = TRUE, raster = FALSE, cols=col_vector) + ggtitle("All samples") + NoLegend()+ylim(-15, 15)+xlim(-15,10)
p3 <- DimPlot(trm1, reduction = "ref.umap", group.by = "predicted.ID", label = TRUE, pt.size=0.2,
	    label.size = 3.5, repel = TRUE, raster = FALSE, cols=col_vector) + ggtitle("All samples") + NoLegend()+ylim(-15, 15)+xlim(-15,10)
p2 | p3


p4 <- DimPlot(trm1, reduction = "ref.umap", group.by = "predicted.ID", label = TRUE, pt.size=0.2, split.by="sample", ncol=7,
	    label.size = 3.5, repel = TRUE, raster = FALSE, cols=col_vector) + ggtitle("All samples") + NoLegend()+ylim(-15, 15)+xlim(-15,10)
p4

# export the Trm1 object
saveRDS(trm1, "/special_projects/55_scalp_psoriasis_analysis_jeff/RData/Trm1_merged_all_samples_integrated_mapping_to_ref.RDS")
